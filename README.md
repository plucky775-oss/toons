...
이전 방식은 선택한 컷 번호를 전달해도 이미지 모델이 2×2 전체 만화를 다시 생성했습니다.

현재 방식:

1. 브라우저가 원본 2×2 이미지에서 선택한 컷만 정확히 잘라냄
2. 선택한 한 컷만 이미지 모델에 전달
3. 이미지 모델은 정사각형 한 컷만 수정
4. 브라우저가 수정된 한 컷을 원본 2×2 이미지의 동일 위치에 합성
5. 나머지 세 컷은 원본 픽셀을 그대로 유지
6. 분리 표시된 컷 이미지도 즉시 갱신


## PNG 저장 범위

`PNG 저장`은 화면 전체를 캡처하지 않습니다.

저장되는 항목:
- AI가 생성한 2×2 4컷 그림 원본
- 컷별 그림 수정 결과
...
따라서 저장 파일은 교육자료 화면이 아니라 순수한 4컷 그림 PNG입니다.

## 화면 모델명 표기 제거

첫 화면과 생성 화면에서 특정 AI 모델 이름을 표시하지 않습니다.

- `AI 안전사고 분석`
- `AI 그림 생성`
- `그림 생성 완료`

모델은 Vercel 환경변수로 관리하며 사용자 화면에는 노출하지 않습니다.


## 포스터형 4컷 교육자료

결과 화면과 PNG를 다음 구성으로 통일합니다.

1. 상단 사고 개요
2. 파란색 핵심 예방 문구
3. 2×2 4컷 카드
4. 각 카드: 컷 번호·제목 → 그림 → 대사 → 검은색 교육포인트
