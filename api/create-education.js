...
[물리·장비 구조 정확성]
- 활선버켓트럭의 붐은 차량 회전대에서 시작해 연속된 붐 구조를 거쳐 버켓에 연결되어야 한다.
- 붐이 작업자의 몸, 팔, 어깨, 안전대에 연결되거나 관통하면 안 된다.
- 작업자는 버켓 내부에 있어야 하며 몸 일부가 버켓 구조를 비현실적으로 통과하면 안 된다.
- 작업자가 전선을 손으로 만지는 장면은 사고보고서에 직접 명시된 경우에만 허용한다.
- 사고 부위가 어깨라면 손·머리·가슴이 전선에 접촉하는 장면으로 바꾸지 않는다.
- 예방컷은 동일한 차량, 동일한 버켓, 동일한 붐, 동일한 전선, 동일한 작업인원을 유지한다.
